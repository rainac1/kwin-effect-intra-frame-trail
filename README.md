...
Intra-frame trail for high-polling-rate mice: a KWin effect that redraws the cursor at
every position sampled during the previous frame. Such a mouse reports far more
positions per frame than the compositor draws, so a moving pointer leaves a trail of
the copies it would otherwise skip; drawing them improves the pointer's visual
smoothness and click accuracy. C++ and OpenGL, built against kwin's effect API. Needs
